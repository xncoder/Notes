...
#+END_SRC
* Vagrant and Docker
** Use Vagrant to build a cluster on local mechine
  1. 用vagrant在本地机器上部署一个3台coreos主机的集群; youtube上有相关视频；
  2. 在集群中的coreos上部署etcd
  3. etcd的简单使用
...
** Config Docker after install on linux
...
** Using Docker build a etcd cluster
* Protobuff
* Kafka
